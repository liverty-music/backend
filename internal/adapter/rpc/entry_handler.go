...
func NewEntryHandler(entryUseCase usecase.EntryUseCase, logger *logging.Logger) *EntryHandler {
	return &EntryHandler{
...
func (h *EntryHandler) GetMerklePath(
	...
	if msg.GetUserId() == nil || msg.GetUserId().GetValue() == "" {
	...
	// Use authenticated user's identity instead of the request body user_id
	// to prevent users from querying other users' Merkle paths.
	_ = claims // TODO: resolve internal user ID from claims.Sub via userRepo

	result, err := h.entryUseCase.GetMerklePath(ctx, msg.GetEventId().GetValue(), msg.GetUserId().GetValue())
	if err != nil {
		return nil, err
	}
