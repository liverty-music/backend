...
type ImageStorer interface {
	...
	// type and returns the public HTTPS URL of the stored object. Objects are
	// content-addressed (the key embeds a hash of the bytes), so a replaced image
	// is written under a NEW key; the caller deletes the prior object via Delete.
	// The stored object is served as immutable, cacheable media.
	//
	// # Possible errors
	//
